...
export function sheetContent({id=uid('sheet'),title='',bodyHTML='',action='닫기',actions,actionAttributes={},headerAttributes={},bodyWrapped=true}={}) {
 return '<div class="og-sheet-header"><h2 id="'+e(id)+'-title" tabindex="-1"'+attrs(headerAttributes)+'>'+e(title)+'</h2>'+iconButton({label:'시트 닫기',name:'close',className:'og-sheet-close',attributes:{'data-sheet-close':true}})+'</div>'+(bodyWrapped?'<div class="og-sheet-body">'+bodyHTML+'</div>':bodyHTML)+'<div class="og-sheet-footer">'+(actions?actions.map(button).join(''):button({label:action,attributes:actionAttributes}))+'</div>';
}
