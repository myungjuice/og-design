...
(async()=>{const b=await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});try{
 ...
 await p.goto(''+(process.env.PREVIEW_URL||'http://127.0.0.1:4173')+'/design-system/canvas/#my-info-notices');await p.waitForFunction(()=>document.querySelector('#viewport')?.getAttribute('aria-busy')==='false');
 assert.equal(await p.locator('#my-info-notices .og-notices').count(),10);
 await p.locator('#my-info-notices [data-screen-prompt]').click({force:true});await p.waitForFunction(()=>!document.querySelector('#screen-prompt-copy').disabled);
 ...
 await p.goto(''+(process.env.PREVIEW_URL||'http://127.0.0.1:4173')+'/design-system/pages/my-info/');
 ...
 for(const width of [320,375,414,768]){await p.setViewportSize({width,height:900});
  for(const [name,props] of [['notice',{}],['notification',{selected:1}],['read',{selected:1,allRead:true}],['edit',{selected:1,editing:true}],['empty',{selected:1,empty:true}],['delete',{selected:1,overlay:'all'}]]){
