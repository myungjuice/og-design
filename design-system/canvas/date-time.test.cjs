...
assert.equal(await p.locator('#date-time .og-calendar').count(),2);assert.equal(await p.locator('#date-time .og-time-picker').count(),2);
...
assert.equal(await p.locator('.og-calendar-day[aria-selected="true"]').count(),1);
assert.equal(await p.locator('.og-calendar-day[aria-selected="true"]').innerText(),'18');
