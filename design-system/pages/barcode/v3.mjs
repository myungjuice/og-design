...
export function barcodeV3Screen(props={}){
 let screen=barcodeScreen(props).replace('class="og-barcode-screen"','class="og-barcode-screen og-barcode-v3"');
 if(props.loggedIn!==false&&(props.available??10000)<=0)screen=screen.replace(button({label:'마일리지 사용'}),button({label:'마일리지 사용',disabled:true}));
 return screen;
}
export function barcodeV3Board(){
 const screen=barcodeV3Screen();
 ...
 ['5,000 M 미만 · 보유 2,400 M / 사용 가능 0 M',{total:2400,available:0}],
 ['보유 마일리지가 없을 때',{total:0,available:0}],
 ['로그인하지 않았을 때',{loggedIn:false}]
 ].map(([title,props])=>'<section class="og-barcode-example"><h3 class="history-state-label">'+title+'</h3><div class="screen-page-content"><div class="screen-artboard">'+barcodeV3Screen(props)+'</div></div></section>').join('');
}
