...
export function account({name='홍길동',phone='010-1234-5678',joined='2024-05-20',mileage=16000,pay=0,visible=false,authenticated=false,confirmation='',assetsOpen=false}={}){
 const confirmed=confirmation==='회원탈퇴';
 const info='<div class="og-account-heading">'+sectionHeading({title:'가입 정보'})+iconButton({label:visible?'개인정보 숨기기':'개인정보 표시',name:visible?'visibility':'visibility_off'})+'</div>'+[['이름',visible?name:maskedName(name)],['전화번호',visible?phone:maskedPhone(phone)],['가입일',joined.slice(0,10)],['보유 마일리지',Number(mileage).toLocaleString('ko-KR')],['보유 페이',Number(pay).toLocaleString('ko-KR')]].map(([title,value])=>listRow({title,value,interactive:false})).join('');
 ...
 const overlay=assetsOpen?'<div class="og-account-overlay">'+dialog({title:'자산 소멸 확인',body:'마일리지, 포인트, 페이, 박스 등의 자산이 남아 있습니다. 회원 탈퇴시 모든 자산은 소멸됩니다.\n\n회원 탈퇴를 진행하시겠습니까?',actions:[{label:'취소',variant:'secondary'},{label:'확인'}]})+'</div>':'';
